...
const SettingsPanel: React.FC<SettingsPanelProps> = ({
  ...
  return (
    <Card className="border flex flex-col h-full overflow-hidden">
      ...
      {/* HEADER: Sticky */}
      <div className="shrink-0 flex items-center justify-between border-b px-4 pb-4">
        ...
        <div className="p-4 space-y-5">
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Select value={mode} onValueChange={(v) => onModeChange(v as TestMode)} disabled={isDisabled}>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={difficulty} onValueChange={(v) => onDifficultyChange(v as Difficulty)} disabled={isDisabled}>
                <SelectTrigger className="h-8 w-full text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={language} onValueChange={(v) => onLanguageChange(v as 'english' | 'khmer' | 'mixed')} disabled={isDisabled}>
                <SelectTrigger className="h-8 w-full text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            {mode === 'time' && (
              <div className="space-y-1.5">
                ...
                <Select value={String(timerDuration)} onValueChange={(v) => onTimerDurationChange(Number(v) as TimerDuration)} disabled={isDisabled}>
                  <SelectTrigger className="h-8 w-full text-xs"><SelectValue /></SelectTrigger>
                  <SelectContent>
            ...
            {mode === 'words' && (
              <div className="space-y-1.5">
                ...
                <Select value={String(wordCount)} onValueChange={(v) => onWordCountChange(Number(v) as 10 | 25 | 50 | 100)} disabled={isDisabled}>
                  <SelectTrigger className="h-8 w-full text-xs"><SelectValue /></SelectTrigger>
                  <SelectContent>
            ...
            {mode === 'practice' && (
              <>
                <div className="space-y-1.5">
                  ...
                  <Select value={practiceTimed ? 'on' : 'off'} onValueChange={(v) => onPracticeTimedChange(v === 'on')} disabled={isDisabled}>
                    <SelectTrigger className="h-8 w-full text-xs"><SelectValue /></SelectTrigger>
                    <SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={soundEnabled ? 'on' : 'off'} onValueChange={(v) => onSoundToggle(v === 'on')}>
                <SelectTrigger className="h-8 w-full text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>
          ...
          {mode === 'custom' && (
            <div className="space-y-1.5 pt-3 border-t border-muted/20">
              ...
              <textarea
                ...
                className="w-full min-h-[100px] border border-input bg-background/50 px-3 py-2 text-xs font-sans resize-y focus:outline-none focus:ring-1 focus:ring-ring"
                disabled={isDisabled}
              />
      ...
      {/* FOOTER: Sticky */}
      <div className="shrink-0 flex flex-col gap-2 px-4 pt-4 border-t">
        <Button
          size="sm"
          variant="outline"
          onClick={onReset}
